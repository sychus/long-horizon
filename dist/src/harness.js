...
const proxy = spawn(process.execPath, ["--import", "tsx", path.join(__dirname, "proxy.ts")], {
...
function send(method, params) {
    ...
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => reject(new Error(`timeout waiting for ${method} (id=${id})`)), 60000);
        waiters.set(id, (msg) => {
            clearTimeout(timer);
            resolve(msg);
        });
...
async function main() {
    ...
    const palaceEvents = wsEvents.filter((e) => e.kind === "palace");
    ...
    const toolsListReq = wsEvents.find((e) => e.kind === "request" && "method" in e && e.method === "tools/list");
    ...
    assert(replayLines.length >= wsEvents.length, `GET /events replay has ${replayLines.length} events (buffer ≥ ws ${wsEvents.length})`);
    // ---- health endpoint ----
