...
function tiles(input) {
    ...
    for (const drawers of scan.byRoom.values()) {
        for (const d of drawers) {
            if (d.parsed.frontmatter.origin === "scan" && d.parsed.frontmatter.reviewed !== true)
                unreviewed++;
        }
    }
    const health = coverage.verifiedRatio >= 0.5
...
function roomSections(input) {
  ...
  <div class="body">${body ? esc(body) : `<span class="none">empty</span>`}</div>
            ...
            .join("\n");
        const unreviewed = drawers.filter((d) => d.parsed.frontmatter.origin === "scan" && d.parsed.frontmatter.reviewed !== true).length;
