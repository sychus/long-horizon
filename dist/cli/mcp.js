...
export function proxyCommandInstalled() {
    try {
 ...
 */
export function expectedEntry(p) {
    return {
 ...
 */
export function writeMcpConfig(file, p) {
...
export function checkMcpConfig(file, p) {
    const config = readMcpConfig(file);
    if (!config)
        return { ok: false, reason: `${path.basename(file)} is missing or is not valid JSON` };
    const entry = config.mcpServers?.[SERVER_NAME];
    if (!entry)
        return { ok: false, reason: `no "${SERVER_NAME}" server registered` };
