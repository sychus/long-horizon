...
import { resolveProject, roomDir, palaceDir } from "../project.js";
import { inspectRepo } from "../inspect.js";
import { renderDrawer, parseDrawer, today } from "../drawer.js";
import { heading, ok, info, warn, out, die, dim, bold, cyan } from "../ui.js";
const list = (items, max = 20) => items.slice(0, max).map((i) => `- \`${i}\``).join("\n") +
    (items.length > max ? `\n- …and ${items.length - max} more` : "");
// ---- drawer construction -------------------------------------------------
function structureDrawer(facts) {
...
function entryPointsDrawer(facts) {
    const { manifest } = facts;
    if (!manifest.file || manifest.entries.length === 0)
        return null;
...
function testTopologyDrawer(facts) {
...
function plan(facts) {
    const planned = [structureDrawer(facts)];
    for (const module of facts.modules) {
        if (module.dir !== ".")
            planned.push(moduleDrawer(module));
    }
...
export async function scan(args) {
    ...
    if (facts.tracked.length === 0) {
        die(`git reported no files in this repository.\n` +
