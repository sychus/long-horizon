...
import { resolveProject, palaceDir, mcpPath, portFor, PALACE_DIR } from "../project.js";
...
export async function doctor() {
    ...
    const report = { failures: 0, warnings: 0 };
...
// ---- taxonomy ------------------------------------------------------------
function checkTaxonomy(project, scan, report) {
    heading("Taxonomy");
...
// ---- drawers -------------------------------------------------------------
function checkDrawers(project, scan, report) {
    heading("Drawers");
    const misfiled = [];
    const broken = [];
    const thin = [];
...
function emit(print, items, summary, fix) {
    if (!items.length)
        return 0;
 ...
 */
function checkCoverage(project, scan, report) {
    heading("Coverage");
    ...
    for (const module of coverage.byModule) {
        const line = `  ${module.dir.padEnd(14)} ${String(module.total).padStart(5)}  ` +
            `${pct(module.mappedRatio).padStart(6)}  ` +
            `${bar(module.verifiedRatio)} ${pct(module.verifiedRatio).padStart(4)}`;
        if (module.verifiedRatio === 0)
            out(`${line}  ${yellow("← nobody has verified this")}`);
        else
            out(line);
    }
    out();
    ...
    if (coverage.unmapped.length > 0) {
        ...
        for (const file of coverage.unmapped.slice(0, 8))
            hint(file);
        if (coverage.unmapped.length > 8)
            hint(`… and ${coverage.unmapped.length - 8} more`);
...
// ---- index ---------------------------------------------------------------
async function checkIndex(project, scan, report) {
    heading("Index");
...
// ---- wiring --------------------------------------------------------------
async function checkWiring(project, report) {
    heading("Wiring");
