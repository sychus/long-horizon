 ...
 */
async function withLockRetry(args, opts, onWait) {
    let result = await mempalace(args, opts);
    for (let attempt = 1; attempt <= LOCK_RETRIES; attempt++) {
        if (result.code === 0 || !isLockContention(result.stdout + result.stderr))
            break;
        onWait(attempt);
...
export async function sync() {
    ...
    const mined = await withLockRetry(["mine", `/work/${PALACE_DIR}`, "--wing", project.slug, "--agent", "palace-cli"], { volume: project.volume, workdir: project.root }, (attempt) => info(`palace busy — another project is syncing (retry ${attempt}/${LOCK_RETRIES})`));
    if (mined.code !== 0) {
        if (isLockContention(mined.stdout + mined.stderr)) {
            die(`The palace stayed locked by another sync after ${LOCK_RETRIES} attempts.\n` +
    ...
    if (filed > 0)
        ok(`${filed} drawer(s) filed`);
    else
        info("nothing new to file");
    if (skipped > 0) {
    ...
    const pruned = await withLockRetry(["sync", "/work", "--wing", project.slug, "--apply"], { volume: project.volume, workdir: project.root }, (attempt) => info(`palace busy — waiting to prune (retry ${attempt}/${LOCK_RETRIES})`));
    if (pruned.code === 0) {
        const removed = (pruned.stdout + pruned.stderr).match(PRUNED)?.[1];
        if (removed && Number(removed) > 0)
            ok(`${removed} stale drawer(s) pruned`);
    }
    else {
