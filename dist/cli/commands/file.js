...
import { renderDrawer, drawerFilename, today, brokenAnchors, MIN_BODY_CHARS, } from "../drawer.js";
...
export async function fileDrawer(args) {
    const project = resolveProject();
    if (!args.room) {
        die(`--room is required. One of: ${ROOM_NAMES.join(", ")}`);
    }
    if (!isRoom(args.room)) {
        die(`"${args.room}" is not a room in this palace.\n` +
    ...
    if (!body) {
        die(`No content. Pass --body "..." or pipe it:\n` +
    ...
    if (broken.length && !args.force) {
        die(`These anchors do not exist in the repo:\n` +
            broken.map((a) => `    ${a}`).join("\n") +
    ...
    writeFileSync(target, renderDrawer({
        room: args.room,
        title: args.title,
    ...
    if (body.length < MIN_BODY_CHARS) {
        warn(`body is ${body.length} chars; drawers under ~${MIN_BODY_CHARS} are skipped by the miner ` +
