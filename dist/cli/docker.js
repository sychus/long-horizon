...
export async function imageExists(image = "mempalace") {
    const { code, stdout } = await run("docker", ["images", "-q", image]);
    return code === 0 && stdout.trim().length > 0;
}
export async function volumeExists(volume) {
...
export async function createVolume(volume) {
    ...
    if (code !== 0)
        throw new Error(`Could not create volume ${volume}: ${stderr.trim()}`);
}
/**
 ...
 */
export function isLockContention(output) {
 ...
 */
export async function mempalace(args, opts) {
    const dockerArgs = ["run", "--rm", "-v", `${opts.volume}:/data`];
    if (opts.workdir)
        dockerArgs.push("-v", `${opts.workdir}:/work:ro`);
