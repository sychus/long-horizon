...
import { fileDrawer, type FileArgs } from "./commands/file.js";
...
async function list(): Promise<void> {
  if (!(await dockerAvailable())) die("Docker is not running.");
...
function parseFileArgs(argv: string[]): FileArgs {
  const args: FileArgs = { anchors: [], force: false };
  for (let i = 0; i < argv.length; i++) {
    const flag = argv[i] ?? "";
    const value = argv[i + 1];
    switch (flag) {
      case "--room": args.room = value; i++; break;
      case "--title": args.title = value; i++; break;
      case "--anchor": if (value) args.anchors.push(value); i++; break;
      case "--body": args.body = value; i++; break;
