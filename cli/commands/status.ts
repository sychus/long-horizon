...
export async function status(): Promise<void> {
  ...
  if (index) {
    ...
    if (neighbours.length) info(`shares this store with ${neighbours.length} other wing(s)`);
  }
  ...
  for (const room of ROOMS) {
    ...
    if (!agree) out(`${line}  ${yellow("← differs")}`);
    else if (room.name === INBOX && onDisk > 0) out(`${line}  ${yellow("← unfiled")}`);
    else out(line);
  }
