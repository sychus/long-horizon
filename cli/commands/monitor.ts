...
type Stream = "sync" | "drift" | "agent" | "info";
...
function log(stream: Stream, message: string): void {
...
function describeEvent(event: ProxyEvent): string | null {
  if (event.kind !== "palace") return null;
  ...
  if (rooms.length) parts.push(dim("→") + " " + rooms.join(", "));
  else parts.push(dim("→ no hits"));

  const cost: string[] = [];
  if (event.tokens != null) cost.push(`${event.tokens} tok`);
  if (event.latencyMs != null) cost.push(`${event.latencyMs}ms`);
  if (cost.length) parts.push(dim(`(${cost.join("  ")})`));

  if (event.isError) parts.push(red("error"));
  return parts.join("  ");
}
 ...
 */
function followProxy(port: number, onClose: () => void): { stop: () => void } {
  ...
  const connect = async (): Promise<void> => {
    ...
    socket.on("message", (data: Buffer) => {
      try {
        const event = JSON.parse(data.toString()) as ProxyEvent;
        const line = describeEvent(event);
        if (line) log("agent", line);
      } catch {
    ...
    const retry = (): void => {
      ...
      socket = null;
      if (!stopped) timer = setTimeout(() => void connect(), 3_000);
    };
...
function anchorIndex(project: Project): Map<string, string[]> {
  ...
  for (const drawers of scan.byRoom.values()) {
    for (const drawer of drawers) {
      for (const anchor of drawer.parsed.frontmatter.anchors ?? []) {
        ...
        if (list) list.push(drawer.rel);
        else index.set(anchor, [drawer.rel]);
      }
    }
  }
  return index;
...
function affectedAnchors(relPath: string, index: Map<string, string[]>): string[] {
  const hits: string[] = [];
  for (const anchor of index.keys()) {
    if (relPath === anchor || relPath.startsWith(`${anchor}/`)) hits.push(anchor);
  }
  return hits;
}
...
export async function monitor(): Promise<void> {
  ...
  const runSync = async (): Promise<void> => {
    ...
    const mined = await mempalace(
      ["mine", `/work/${PALACE_DIR}`, "--wing", project.slug, "--agent", "palace-monitor"],
  ...
  watchers.push(
    watch(palaceDir(project), { recursive: true }, (_event, filename) => {
  ...
  watchers.push(
    watch(project.root, { recursive: true }, (_event, filename) => {
      if (!filename) return;
      const rel = filename.toString();
      if (rel.startsWith(PALACE_DIR) || rel.startsWith(".git")) return;
