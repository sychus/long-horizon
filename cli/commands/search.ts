...
export interface SearchArgs {
  ...
  room?: string;
  results?: number;
  /** Search every project in the shared store, not just this one. */
  allWings: boolean;
}
...
export async function search(args: SearchArgs): Promise<void> {
  ...
  if (!args.allWings) argv.push("--wing", project.slug);
  if (args.room) argv.push("--room", args.room);
  if (args.results) argv.push("--results", String(args.results));
