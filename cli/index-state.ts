...
export interface IndexState {
  /** Total drawers across every wing in the shared palace. */
  total: number;
  /** Drawer count per room, for the wing we asked about. */
  rooms: Map<string, number>;
  /** Every wing in the shared palace — one per project, plus any legacy wings. */
  wings: string[];
}
 ...
 */
export const wingTotal = (state: IndexState): number =>
  [...state.rooms.values()].reduce((sum, n) => sum + n, 0);

const TOTAL = /Status\s+[—-]\s+(\d+)\s+drawers/;
const WING = /^\s*WING:\s*(\S+)/;
const ROOM = /^\s*ROOM:\s*(\S+)\s+(\d+)\s+drawers/;

export function parseStatus(output: string, wing: string): IndexState {
  const state: IndexState = { total: 0, rooms: new Map(), wings: [] };

  const total = output.match(TOTAL);
  if (total) state.total = Number(total[1] ?? 0);
...
export async function readIndexState(volume: string, wing: string): Promise<IndexState | null> {
  const { code, stdout, stderr } = await mempalace(["status"], { volume });
  if (code !== 0) return null;
  const output = stdout + stderr;
  if (!TOTAL.test(output)) return null;
