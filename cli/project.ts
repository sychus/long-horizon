...
export interface Project {
  ...
  root: string;
  /** Directory name of the root, unmodified. */
  name: string;
  /** Normalized identifier, used as this project's wing name. */
  slug: string;
  /** Docker named volume — shared by every project on this machine. */
  volume: string;
  /** WebSocket port for this project's proxy. Still per-project: each Claude
   *  Code session runs its own proxy and the extension must find the right one. */
  port: number;
}
...
export function slugify(name: string): string {
  const slug = name
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  if (!slug) throw new Error(`Cannot derive a palace slug from "${name}"`);
  return slug;
}
 ...
 */
export function portFor(slug: string): number {
  let hash = 0x811c9dc5;
 ...
 */
export function resolveProject(cwd: string = process.cwd()): Project {
  const root = findRepoRoot(cwd);
  if (!root) {
    throw new Error(
      `No git repository found at or above ${cwd}.\n` +
...
export const palaceDir = (p: Project): string => path.join(p.root, PALACE_DIR);
export const roomDir = (p: Project, room: string): string => path.join(palaceDir(p), room);
export const configPath = (p: Project): string => path.join(p.root, CONFIG_FILE);
export const mcpPath = (p: Project): string => path.join(p.root, MCP_FILE);
